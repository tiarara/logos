...
## 3c. House (Ruby the cleaner, Ron the maintenance retainer)
...
- Retainer, this calendar month: DAY rows = full days used of 4; PICKUP rows count. REPAIR with `due` before today and `completed` empty → Needs you ("{description} is N days past due"). DAY at 4 of 4 before the 20th, or 0 of 4 on/after the 20th → one Needs you line. PICKUP dated yesterday → Sorted with its source. REPAIR completed yesterday → Sorted.
